...
const decideOptions = {
  ...
  STOP: "Vuole interrompere la conversazione con il bot",
  OTHER:
  ...
  return new Promise((resolve, reject) => {
    const client = ollama
      ...
      .then(async (stream) => {
        ...
        const s = pipeline(stream, textTransform, () => {
          onEnd?.(response.join(""));
        });
...
export async function decide(content, { signal }) {
  ...
  const response = await ollama.chat({
    ...
    messages: [
      {
        role: "system",
        content: "Trovare l'intento dell'utente in base al testo fornito"
      },
      { role: "user", content }
    ],
    options: {
